...
root = Path.cwd()
manifest_path = root / "experience/organic-v0/cycle11_probe_states_v0.jsonl"
for line_no, line in enumerate(manifest_path.read_text().splitlines(), 1):
    if not line.strip():
        continue
    record = json.loads(line)
    parent = record.get("parent_state_path")
    child = record.get("state_path")
    if not parent or not child:
        raise SystemExit(f"{manifest_path}:{line_no}: parent_state_path and state_path are required")
    subprocess.run([
        "timeout", "180s", "build/organic_v0",
        "--phase", "state-config-copy",
        "--mode", "test",
        "--run-id", f"cycle11-cap160-{record['state_id']}",
        "--load-state", parent,
        "--save-state", child,
...
state_manifest_path = root / "experience/organic-v0/cycle11_probe_states_v0.jsonl"
...
def pxstate_config(path):
    config = {}
    state_hash = None
    for line in path.read_text().splitlines():
        if line.startswith("CONFIG "):
            for token in line[len("CONFIG "):].split():
                if "=" in token:
                    key, value = token.split("=", 1)
                    config[key] = value
        elif line.startswith("STATE_HASH "):
            state_hash = line.split(maxsplit=1)[1]
    return config, state_hash

prompt_records = []
allowed_prompt_keys = {"schema", "prompt_id", "prompt_text", "observations", "source"}
for line_no, line in enumerate(prompt_path.read_text().splitlines(), 1):
    if not line.strip():
        continue
    record = json.loads(line)
    prompt_records.append(record)
    forbidden = {"label", "labels", "correction_output", "expected_output", "target_output", "reward", "score", "composition", "topic", "intent", "theme", "expected_filler"}
    present = sorted(forbidden.intersection(record))
    check(f"prompt_{line_no}_unlabeled", not present, f"forbidden keys: {present}")
    unexpected = sorted(set(record) - allowed_prompt_keys)
    check(f"prompt_{line_no}_allowed_keys_only", not unexpected, f"unexpected keys: {unexpected}")
    labelish_observations = [obs for obs in record.get("observations", []) if obs.startswith(("theme:", "topic:", "intent:", "expected_filler:"))]
    check(f"prompt_{line_no}_raw_observations_only", not labelish_observations, f"label-shaped observations: {labelish_observations}")
    check(f"prompt_{line_no}_schema_v1", record.get("schema") == "project_x.philosophy_prompt.v1", record.get("schema", ""))

probe_records = []
for line_no, line in enumerate(state_manifest_path.read_text().splitlines(), 1):
    if not line.strip():
        continue
    record = json.loads(line)
    probe_records.append(record)
    check(f"probe_state_{line_no}_schema_v1", record.get("schema") == "project_x.cycle11_probe_state.v1", record.get("schema", ""))
    parent_path = root / record["parent_state_path"]
    child_path = root / record["state_path"]
    parent_config, parent_hash = pxstate_config(parent_path)
    child_config, child_hash = pxstate_config(child_path)
    check(f"probe_state_{line_no}_parent_cap_legacy", parent_config.get("max_output_chars") == "48", str(parent_config.get("max_output_chars")))
    check(f"probe_state_{line_no}_child_cap_persisted", child_config.get("max_output_chars") == "160", str(child_config.get("max_output_chars")))
    check(f"probe_state_{line_no}_cap_changes_state_hash", parent_hash != child_hash, f"{parent_hash} -> {child_hash}")
...
check("artifact_schema", artifact["schema"] == "project_x.quote_per_state_cycle11.v1", artifact.get("schema", ""))
...
check("runtime_budget_not_override", artifact["generation_budget"]["runtime_override_applied"] is False and artifact["generation_budget"]["runtime_max_output_chars"] == 0, str(artifact["generation_budget"]))
check("persisted_budget_required", artifact["generation_budget"]["loaded_state_config_required"] is True and artifact["generation_budget"]["required_persisted_max_output_chars"] == 160, str(artifact["generation_budget"]))
...
check("audit_status_ungraded", artifact["audit_status"] == "ungraded; rubric-pending for GPT/lain audit", artifact.get("audit_status", ""))
missing_negative = sorted(k for k in required_negative_space if artifact["negative_space"].get(k) is not True)
check("negative_space_complete", not missing_negative, f"missing/false: {missing_negative}")
...
for state in states:
    ...
    check(f"{state['state_id']}_loaded_cap_persisted", state["loaded_max_output_chars"] == 160, str(state["loaded_max_output_chars"]))
    check(f"{state['state_id']}_active_cap_from_disk", state["active_max_output_chars"] == 160, str(state["active_max_output_chars"]))
    for output in state["outputs"]:
        ...
        check(f"{state['state_id']}_{output['prompt_id']}_audit_status", output["audit_status"] == "ungraded; rubric-pending for GPT/lain audit", output.get("audit_status", ""))
        for forbidden_key in ["self_score", "self_grade", "semantic_score", "quality_score", "subjective_score"]:
            check(f"{state['state_id']}_{output['prompt_id']}_no_{forbidden_key}", forbidden_key not in output, forbidden_key)
...
result = {
    "schema": "project_x.cycle11_voice_pressure_verification.v1",
    ...
    "state_manifest_path": str(state_manifest_path.relative_to(root)),
    "negative_space": {key: True for key in sorted(required_negative_space)},
    "checks": checks,
