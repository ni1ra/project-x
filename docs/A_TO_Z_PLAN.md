...
Honest boundary for v3-c9: In-process policy enforcement; wrapper-level sandbox is a future cycle. This does not solve alignment, AGI safety, sandbox escape resistance, or broader tool-use safety. The v2 event-log chain is internal to a log and needs wrapper-level head/length anchoring before claiming truncate-and-restart resistance.
