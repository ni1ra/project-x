...
| **#00P13c10-02-diophantine-pell-extension** | MED (design shipped; implementation NEXT) | `src/project_x/reasoning/diophantine.py` — implement per `docs/artifacts/cycle-10-pell-design.md` | **Design doc** at `docs/artifacts/cycle-10-pell-design.md` covers: `_continued_fraction_sqrt(n)` helper (Hardy+Wright recurrence on (m, d, a) triples; period detection by triple-repetition); `_pell_fundamental_solution(n)` (convergent (p_{p-1}, q_{p-1}) for even-period; squared (a + b√n)² for odd-period); `solve_pell_equation(n, k_max)` Lemma chain (4 steps: validate_input → compute_continued_fraction → compute_fundamental_solution → apply_recurrence); STRONG verifier via brute-force search for fundamental (capped at M=1000); dispatcher extension to route Pell-shape (a=1, b=0, c=-n, N=1); ladder entries maths-026 (n=2 → (3,2) classic period-odd-squared) + maths-027 (n=3 → (2,1) period-even-direct). Open questions list at end. Implementation cost estimate: ~60 min Raphael-time. Pickup: this-instance after context reset OR fresh-instance via `/forge-prompt -ni` corpse anchored at the design doc. |
