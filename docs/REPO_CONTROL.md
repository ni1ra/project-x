...
| Path | Justification |
|---|---|
| `src/project_x/reasoning/__init__.py` | Package marker + identity-discipline note (the AGENT's reasoning layer, not the BUILDER's). Re-exports `DerivationStep`, `InvariantCheck`, `Lemma`, `solve_quadratic`, `expand_characteristic_polynomial_2x2`, `numerical_verify`, `polynomial_definite_integral`, `residue_theorem_unit_quadratic`, `first_order_linear_ode_exp_solution`, plus `INTRO_*` math-WHY prose constants. Cycle 8 #03 retrofit added complex_analysis + calculus + ode re-exports (cycle 8 #01/#02 substrate commits had imported via submodule path only). |
...
| `src/project_x/reasoning/calculus.py` | Phase 13 cycle 8 #00P13c8-02 — from-scratch definite-integral evaluator via Fundamental Theorem of Calculus. Closes maths-010 agent-runtime gap. `polynomial_definite_integral(coeffs_descending, lower, upper)` computes ∫_lower^upper p(x) dx for polynomial p of any degree (coefficients in descending order matching natural reading: [3, 2, -1] = 3x² + 2x - 1). 4-step Lemma chain (antiderivative-via-power-rule → evaluate_upper-via-Horner → evaluate_lower-via-Horner → ftc_closing). Invariant: (F(upper)+K) - (F(lower)+K) = F(upper) - F(lower) for any K — constant-of-integration cancellation verifies subtraction implementation. Empty coefficient list = zero polynomial (integral 0). Hand-rolled stdlib only — NO sympy, NO scipy, NO numerical-integration (organic-thesis binding). Cycle 8+ extends to transcendental integrands (sin/cos/exp/log) as ladder entries require. |
| `src/project_x/reasoning/complex_analysis.py` | Phase 13 cycle 8 #00P13c8-01 — from-scratch residue-theorem-based real-line integral evaluator. Closes maths-003 agent-runtime gap (the residue theorem entry the cycle-7 ReasoningAgent couldn't solve). `residue_theorem_unit_quadratic(a, c)` computes ∫_(-∞)^(+∞) dx/(a·x² + c) = π/√(a·c) via closed-form residue theorem on upper-half-plane contour. 3-step Lemma chain (locate_poles → compute_residue → residue_theorem_closing) + dimensionless invariant (integral·√(a·c)/π = 1 universal). Hand-rolled stdlib `math.sqrt` + `math.pi` only — NO scipy, NO sympy, NO numerical-integration libraries (organic-thesis binding). Cycle 8+ extends to higher-degree denominators + non-trivial numerators as ladder entries require. |
| `src/project_x/reasoning/ode.py` | Phase 13 cycle 8 #00P13c8-03 — from-scratch first-order linear separable homogeneous ODE solver. Closes maths-011 agent-runtime gap. `first_order_linear_ode_exp_solution(k, y_0, x_target, x_0=0.0)` computes closed-form y(x) = y_0·e^(k·(x - x_0)) at x_target for ODE dy/dx = k·y with IC y(x_0) = y_0. 4-step Lemma chain (identify_form → separate_variables → integrate_and_apply_IC → evaluate_at_target). Inline algebraic-identity invariant (IC preservation y(x_0) = y_0) matching cycle 8 #01/#02 pattern; verifier.py ODE branch + independent-path Taylor verifier deferred to cycle 9 predicate-strength uniformity pass per advisor cycle-8 #03 verdict 2026-05-11. Hand-rolled stdlib `math.exp` only — NO scipy, NO sympy, NO numerical-ODE-solver libraries (organic-thesis binding). Trivial branches (y_0=0 → ≡0; k=0 → ≡y_0) handled naturally by closed-form. Cycle 8+ extends to non-homogeneous (dy/dx + p(x)·y = q(x)) and separable-general (dy/dx = f(x)·g(y)) as ladder entries require. |
| `src/project_x/reasoning/physics.py` | Phase 13 #00P13c4-02 — from-scratch closed-form physics primitives mirroring the cycle 2/3 maths substrate shape. Cycle 4 Tier 2 (3 primitives): `free_fall_time(h, g)` (kinematic identity h = ½gt² → t = √(2h/g); 1-step + energy-conservation invariant v_f = g·t = √(2gh)); `pendulum_period(L, g)` (small-angle SHM via Lagrangian linearization → T = 2π√(L/g); 1-step + dimensionless-universal invariant T²·g/L = 4π²); `relativistic_momentum(m, v, c)` (Lorentz factor γ = 1/√(1-β²) + p = γmv; 2-step + Newtonian-limit invariant γ - 1 ≈ β²/2). **Cycle 5 #00P13c5-05 Tier 3 extension:** `large_angle_pendulum_period(L, g, theta_0)` extends pendulum beyond small-angle linearization via complete-elliptic-integral series (truncated at k⁸; 4 terms; ~3e-3 accuracy at θ₀ = π/2 vs textbook reference). Hand-rolled summation; recursion `a_{n+1}/a_n = ((2n+1)/(2n+2))²·k²` validates via algorithmically-distinct ratio-recursion in tests. Each lemma carries `add_introduction` math-WHY prose (kinematic / Lagrangian / SR / elliptic-integral derivations) + `add_invariant_check` post-derivation. Hand-rolled — uses only `math.sqrt` + `math.pi` + `math.sin` (Python stdlib); NO scipy.constants, NO numerical-physics libraries. Anti-cheat-aware design (cycle 4 #00P13c4-24 + cycle 5 #00P13c5-01): each primitive computes per-input; predicate-strength STRONG across all primitives (cycle 5 #01 closed asymmetry on `relativistic_momentum`); large-angle pendulum predicate uses ratio-recursion form independent of substrate's explicit-coefficient sum. Raises ValueError on non-positive inputs, superluminal velocity (`relativistic_momentum`), full-inversion regime |θ₀| ≥ π (`large_angle_pendulum_period`). |

### Project X Raphael agent runtime — `src/project_x/reasoning_agent.py` (Phase 13 #00P13c7-NEW)

| Path | Justification |
|---|---|
| `src/project_x/reasoning_agent.py` | Phase 13 cycle 7 #00P13c7-NEW — rule-based AGENT runtime that consumes benchmark prompts + dispatches to existing substrate primitives, returning computed Lemma renders (operationalizes lain mid-cycle-7 directive 2026-05-10: *"make the agent actually solve the problems ... real results"*). `ReasoningAgent.process(prompt: str) -> AgentResponse` tries each problem-shape parser in turn; first match wins. Cycle 7 minimum-viable scope: maths quadratic (covers maths-001/007/008 — 3 standard-form-quadratic ladder entries). `_parse_quadratic` regex matches `a x^2 [+-] b x [+-] c = 0` form with optional whitespace + implicit-1-coefficient handling. `_parse_signed_coefficient` normalizes regex match strings to floats (empty/+/- means implicit 1 or -1 per implicit_one flag). Dispatcher branch routes parsed (a,b,c) to existing `solve_quadratic` substrate; lemma.render() becomes answer_text. Honest failure mode: AgentResponse(confidence='refused') with reason when no parser matches (M-PROJECTX-013 measure-don't-claim — no confabulation). Organic-thesis compliance: NO LLM calls, NO pretrained-transformer derivatives; pure regex + hand-coded dispatcher. Cycle 7+ extends to 2x2 eigenvalues / integrals / ODEs / 3x3 determinants / physics primitives. Each extension = new `_try_<shape>()` method + new dispatch chain entry. |
