...
| `run/artifacts/organic-v0/eval_compositional_v2c3.json` | cycle-3 headline: tightened-benchmark eval with the learned segment-generation mechanism shipped. Overall exact_rate `0.680` (+0.32 over cycle-2 baseline 0.360). unseen_filler 8/8, unseen_filler_long 2/2 (both 0/8 + 0/2 in cycle 2). Honest direct_replay regression 5/5 → 3/5 documented in the cycle reflection; cycle-4 scope. State hash `4962e498de53e6c4` |
| `run/artifacts/organic-v0/eval_compositional_v2c3_from_disk.json` | cycle-3 verification: fresh-process eval loaded from disk (no JSONL training), summary_metrics + model_state_hash diff-clean against the from-JSONL eval. Proves the new segment-mode state survives the persistence round-trip |
| `run/artifacts/organic-v0/persist_self_test_v2c3.json` | cycle-3 persistence-round-trip verdict: parent state_hash + raw_output `"mila quartz pier6"` matches child bit-exactly; `load_status: save_and_load_verified`. Confirms ROLES + SEGMENT_CONNS sections serialize and load cleanly under the new state_hash gate |
